...
function ensureLocalExtensionFiles() {
  try {
    const localAppData = process.env.LOCALAPPDATA || '';
    if (!localAppData) return null;

    const targetDir = path.join(localAppData, 'GatilhoBR', 'chrome-extension');
    const targetManifestPath = path.join(targetDir, 'manifest.json');
    const currentAppVersion = app ? app.getVersion() : '4.3.0';

    let needsSync = true;
    if (fs.existsSync(targetManifestPath)) {
      try {
        const m = JSON.parse(fs.readFileSync(targetManifestPath, 'utf8'));
        if (m.version === currentAppVersion) {
          needsSync = false;
        }
      } catch (_) {
        needsSync = true;
    ...
    // Procura a melhor fonte dos arquivos da extensão
    ...
    const sourceDir = sourceCandidates.find((d) => {
      try {
    ...
    if (!sourceDir) {
      console.warn('[Extension Sync] Nenhuma fonte da extensão com manifest.json encontrada.');
      return null;
    }

    console.log(`[Extension Sync] Sincronizando extensão de "${sourceDir}" para "${targetDir}"...`);
    ...
    console.log(`[Extension Sync] ✅ Extensão v${currentAppVersion} sincronizada com sucesso em: ${targetDir}`);
    return targetDir;
  } catch (err) {
    console.error('[Extension Sync] ❌ Erro ao sincronizar extensão local:', err.message);
    return null;
  }
}
...
function detectExtensionInChromiumProfiles() {
  ...
  // Fallback: Diretório local sincronizado no disco
...
ipcMain.handle('gbr:get-installation-status', async () => {
  ...
  const extensionInstalled = Boolean(detectedExtension?.inBrowser || detectedExtension?.installed);
  const extensionActive = Boolean(
    ...
    (detectedExtension?.inBrowser && detectedExtension?.enabled)
  );
  const extensionVersion = lastSeenExtensionVersion || (detectedExtension?.inBrowser ? detectedExtension?.version : null) || (extensionActive ? appVersion : null);
